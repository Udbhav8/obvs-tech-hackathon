...
export default function FloatingChatbot(): JSX.Element {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: "bot",
      text: "Hi! I am your MongoNext assistant. How can I help you?",
    },
  ]);
  ...
  const handleSend = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        context = data.results.map((r: any) => r.text).join("\n\n");
      }
    ...
    <Box sx={{ position: "fixed", bottom: 24, right: 24, zIndex: 1300 }}>
      ...
      <Fade in={open} unmountOnExit>
        <Paper
          elevation={8}
          sx={{
            width: expanded ? 600 : 340,
            height: expanded ? 700 : 420,
            display: "flex",
        ...
        >
          ...
          >
            <Avatar
              sx={{ bgcolor: "secondary.main", mr: 1, width: 36, height: 36 }}
            >
              <Image
                src="/images/logo-circle-white-on-black.png"
                alt="MongoNext Logo"
