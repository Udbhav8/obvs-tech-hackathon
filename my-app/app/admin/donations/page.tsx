...
const RECEIPT_TYPES: { value: ReceiptType; label: string }[] = [
    ...
    { value: "Sent via CanadaHelps", label: "Sent via CanadaHelps" },
...
export default function DonationsPage() {
    ...
    const [newDonationData, setNewDonationData] = useState({
        donation_type: DONATION_TYPES[0].value, // Default to Personal
        donor_name: "",
        donation_amount: 0,
        eligible_amount: 0,
        value_advantage: 0,
        payment_type: PAYMENT_TYPES[0].value, // Default to Cheque
        receipt_type: RECEIPT_TYPES[0].value, // Default to Required
    ...
    const handleNewDonationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setNewDonationData((prev) => ({
            ...prev,
            [id]: id === "donation_amount" || id === "eligible_amount" || id === "value_advantage" ? parseFloat(value) : value,
        }));
    };
    ...
    const handleSaveDonation = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.message.includes("Failed to create donation") && errorData.error.includes("donor_name")) {
            ...
            setNewDonationData({
                donation_type: DONATION_TYPES[0].value,
                donor_name: "",
                donation_amount: 0,
                eligible_amount: 0,
                value_advantage: 0,
                payment_type: PAYMENT_TYPES[0].value,
                receipt_type: RECEIPT_TYPES[0].value,
                received_date: new Date(),
                processed_date: new Date(),
                deposit_date: new Date(),
            });
    ...
    const filteredDonations = useMemo(() => {
        return donations.filter((donation) => {
            const matchesType = activeTab === "all" || donation.donation_type === activeTab;
            const matchesPaymentType = paymentTypeFilter === "all" || donation.payment_type === paymentTypeFilter;
            const matchesSearchTerm = searchTerm === "" ||
                (donation.donor_id?.first_name && donation.donor_id.first_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (donation.donor_id?.last_name && donation.donor_id.last_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (donation.donor_id?.organization_name && donation.donor_id.organization_name.toLowerCase().includes(searchTerm.toLowerCase()));
            ...
            const matchesReceivedDate = (!receivedDateRange.from || receivedDate >= receivedDateRange.from) &&
                                        (!receivedDateRange.to || receivedDate <= receivedDateRange.to);
            const matchesProcessedDate = (!processedDateRange.from || processedDate >= processedDateRange.from) &&
                                         (!processedDateRange.to || processedDate <= processedDateRange.to);
            const matchesDepositDate = (!depositDateRange.from || depositDate >= depositDateRange.from) &&
                                       (!depositDateRange.to || depositDate <= depositDateRange.to);

            return matchesType && matchesPaymentType && matchesSearchTerm &&
                   matchesReceivedDate && matchesProcessedDate && matchesDepositDate;
        });
    }, [donations, activeTab, paymentTypeFilter, searchTerm, receivedDateRange, processedDateRange, depositDateRange]);


    return (
        <div className="container mx-auto py-6 space-y-6 font-inter"> {/* Applied font-inter */}
                ...
                <CardContent>
                    <div className="space-y-4">
                        <div className="flex items-center space-x-2">
                            ...
                            />
                            <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className="rounded-md"> {/* Added rounded-md */}
                        ...
                        {showFilters && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border rounded-lg bg-gray-50"> {/* Responsive grid, rounded-lg, bg-gray-50 */}
                                <div className="space-y-2">
                                    ...
                                    >
                                        <SelectTrigger id="paymentTypeFilter" className="rounded-md">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2"> {/* Responsive flex */}
                                        <Popover>
                                        ...
                                        <Table>
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    <TableHead className="min-w-[140px]">Processed Date</TableHead>
                                                    <TableHead className="min-w-[140px]">Deposit Date</TableHead>
                                                ...
                                                {filteredDonations.length === 0 ? (
                                                    ...
                                                    filteredDonations.map((donation) => (
                                                        <TableRow key={donation._id}>
                                                            ...
                                                            <TableCell>{getCategoryBadge(donation.donation_type)}</TableCell>
                                                            <TableCell>{donation.payment_type}</TableCell>
                                                            <TableCell>{format(new Date(donation.received_date), "PPP")}</TableCell>
                                                            <TableCell>{format(new Date(donation.processed_date), "PPP")}</TableCell>
                                                            <TableCell>{format(new Date(donation.deposit_date), "PPP")}</TableCell>
                    ...
                    <CardContent>
                        <form className="space-y-6" onSubmit={handleSaveDonation}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {/* Responsive grid */}
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value: DonationType) => handleSelectChange("donation_type", value)}
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        type="number"
                                        placeholder="0.00"
                                        value={newDonationData.donation_amount === 0 ? "" : newDonationData.donation_amount}
                                        onChange={handleNewDonationChange}
                                        step="0.01"
                                        className="rounded-md"
                                    />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        type="number"
                                        placeholder="0.00"
                                        value={newDonationData.eligible_amount === 0 ? "" : newDonationData.eligible_amount}
                                        onChange={handleNewDonationChange}
                                        step="0.01"
                                        className="rounded-md"
                                    />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        type="number"
                                        placeholder="0.00"
                                        value={newDonationData.value_advantage === 0 ? "" : newDonationData.value_advantage}
                                        onChange={handleNewDonationChange}
                                        step="0.01"
                                        className="rounded-md"
                                    />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value: PaymentType) => handleSelectChange("payment_type", value)}
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value: ReceiptType) => handleSelectChange("receipt_type", value)}
                                    >
                                        ...
                                        <PopoverTrigger asChild>
                                            <Button
                                                ...
                                                className={cn(
                                                    ...
                                                    !newDonationData.received_date && "text-muted-foreground"
                                                )}
                                            >
                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                {newDonationData.received_date ? format(newDonationData.received_date, "PPP") : "Pick a date"}
                                            </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0 rounded-md">
                                            <Calendar
                                                ...
                                                onSelect={(date) => handleDateChange("received_date", date)}
                                                initialFocus
                                            />
                                        </PopoverContent>
                                    </Popover>
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button
                                                ...
                                                className={cn(
                                                    "w-full justify-start text-left font-normal rounded-md",
                                                    !newDonationData.processed_date && "text-muted-foreground"
                                                )}
                                            >
                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                {newDonationData.processed_date ? format(newDonationData.processed_date, "PPP") : "Pick a date"}
                                            </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0 rounded-md">
                                            <Calendar
                                                ...
                                                onSelect={(date) => handleDateChange("processed_date", date)}
                                                initialFocus
                                            />
                                        </PopoverContent>
                                    </Popover>
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button
                                                ...
                                                className={cn(
                                                    "w-full justify-start text-left font-normal rounded-md",
                                                    !newDonationData.deposit_date && "text-muted-foreground"
                                                )}
                                            >
                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                {newDonationData.deposit_date ? format(newDonationData.deposit_date, "PPP") : "Pick a date"}
                                            </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0 rounded-md">
                                            <Calendar
                                                ...
                                                onSelect={(date) => handleDateChange("deposit_date", date)}
                                                initialFocus
                                            />
                                        </PopoverContent>
                                    </Popover>
                            ...
                            <div className="flex justify-end space-x-4">
                                <Button variant="outline" onClick={() => setShowNewDonation(false)} type="button" className="rounded-md">
                                    Cancel
                                </Button>
            ...
            <Dialog open={showDonorPrompt} onOpenChange={setShowDonorPrompt}>
                <DialogContent className="sm:max-w-[425px] rounded-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            The donor "{newDonationData.donor_name}" was not found in the database.
                            Would you like to create a new donor profile for this name?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setShowDonorPrompt(false)} className="rounded-md">Cancel</Button>
                        <Button onClick={async () => {
                            setShowDonorPrompt(false);
                            // Re-attempt save, backend will now create the donor
                            await handleSaveDonation(new Event('submit') as unknown as React.FormEvent);
                        }} className="rounded-md">Create Donor & Save</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
